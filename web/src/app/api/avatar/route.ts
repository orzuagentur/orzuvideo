...
/** Save HeyGen avatar style for the Avatar video generator. */
export async function POST(request: Request) {
  ...
  const payload = {
    ...
    heygen_background_mode: String(body.heygen_background_mode || "none"),
    ...
    niche: "avatar",
    style_prompt:
      "Friendly talking-head creator. Clear, natural speech for vertical video.",
  };
  ...
  const { error } = await supabase.from("instagram_training").upsert(
    {
      ...payload,
      niche: existing?.niche || payload.niche,
      style_prompt: existing?.style_prompt || payload.style_prompt,
    },
    { onConflict: "user_id" },
  );
