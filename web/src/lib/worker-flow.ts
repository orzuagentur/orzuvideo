...
export const FLOW_NODES: FlowNodeDef[] = [
  ...
  },
  {
    ...
    col: 3,
    row: 3,
    title: "Background music (own R2 library)",
    how: "Picks an unused track from your Music library by genre + mood, downloads from Cloudflare R2, mixes under voice. Upload tracks on the Music page.",
    connects: ["Input: genre / mood / preferred track ids", "Output: music.mp3"],
    needs: ["R2_* env", "Tracks in music_tracks"],
    tips: ["Create genres and upload licensed MP3s first"],
    integrationId: "library",
  },
  {
    id: "ffmpeg",
  ...
  },
  {
    ...
    col: 2,
    row: 4,
    title: "Cloudflare R2 preview",
    how: "Uploads the finished MP4 to Cloudflare R2 at {user_id}/{job_id}.mp4, sets preview_url + storage_path. Creativity playback uses signed URLs via /api/jobs/[id]/preview.",
...
export const FLOW_EDGES: FlowEdge[] = [
  ...
  { from: "elevenlabs", to: "library" },
  { from: "pexels", to: "ffmpeg", label: "4 edit" },
  { from: "library", to: "ffmpeg" },
