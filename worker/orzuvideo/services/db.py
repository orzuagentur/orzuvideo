...
def claim_next_instagram_job(sb: Client) -> dict[str, Any] | None:
    now = datetime.now(timezone.utc).isoformat()
    result = (
        sb.table("instagram_jobs")
        .select("*")
        .eq("status", "queued")
        .lte("scheduled_for", now)
        .order("scheduled_for")
...
def update_ig_job(sb: Client, job_id: str, **fields: Any) -> None:
    sb.table("instagram_jobs").update(fields).eq("id", job_id).execute()
...
def get_instagram_account(sb: Client, user_id: str) -> dict[str, Any] | None:
    result = (
